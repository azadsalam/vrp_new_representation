		...
		for(int period = 0; period<problemInstance.periodCount; period++)
		{
			...
			for(int client=0;client<problemInstance.customerCount;client++)
			{
				...
				if(temp1.periodAssignment[period][client]==true)
				{
					...
					{
						int closestDepot = closestDepot(client);
						ArrayList<Integer> vehicles = problemInstance.vehiclesUnderThisDepot.get(closestDepot);
						int ran = Utility.randomIntInclusive(vehicles.size()-1);
						int vehicle = vehicles.get(ran);
				...
				if(temp2.periodAssignment[period][client]==true)
				{
					if(doesRouteContainThisClient(problemInstance, temp2, period, client)==false)
					{
						...
						temp2.routes.get(period).get(vehicle).add(client);
					}
				}
			}
