...
public class Scheme8_with_single_threshold_Archive_Truncation implements GeneticAlgorithm
{
	...
	public Scheme8_with_single_threshold_Archive_Truncation(ProblemInstance problemInstance) 
	{
...
//		double bestBeforeInjection=-1;
		...
		for( generation=0;generation<NUMBER_OF_GENERATION && failure < 13;generation++)
		{
			...
			else if(unImprovedGeneration >= 75)
			{
				injectionSize = 0 ;
				unImprovedGeneration = 0;	
				
				int elitistRatio = POPULATION_SIZE / 20;
			...
			for(i=1;i<=injectionSize;i++)
			{
				population[POPULATION_SIZE - i] = new Individual(problemInstance);
				population[POPULATION_SIZE - i].initialise();
			}
